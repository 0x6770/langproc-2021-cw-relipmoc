...
Function::Function(std::string _type, std::string _name, ProgramPtr _statements, ProgramPtr _Arguments, int _pos){
   type = _type;
   name = _name;
...
int Function::codeGen(const Binding &_binding, int reg) const {
  ...
  if(with_param == 1){Arguments->codeGen(temp,2);}
...
int Param::evaluate(const Binding &_binding) const{
  return 0;
}
...
int Paramlist::evaluate(const Binding &_binding) const{
  return 0;
}
